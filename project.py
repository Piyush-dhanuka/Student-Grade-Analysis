...
def calculate_key_stats(df):
    ...
    print(f"Overall Class Average: {stats['Overall_Class_Average']:.2f}")
    print(f"Students Passed: {stats['Pass_Count']}")
    print("Subject Averages:")
    for subject, avg in stats['Subject_Averages'].items():
        print(f"  {subject}: {avg:.2f}")
    print(f"Unique Statuses: {', '.join(stats['Unique_Statuses'])}")
...
def generate_visualization(df):
    ...
    sns.barplot(x='Subject', y='Average_Mark', data=subject_means, palette='viridis', hue='Subject', legend=False)
